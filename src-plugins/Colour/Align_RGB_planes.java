...
implements ActionListener, AdjustmentListener, ItemListener, WindowListener, Runnable {
	...
	public Align_RGB_planes () {
		...
		if (imp.getType() != ImagePlus.COLOR_RGB) {
			IJ.showMessage("Align RGB planes.\n \nA 24bit RGB image is required.\nUse Align Slice with 48bit RGB images.");
			return;
		}
...
//		bnClose = new Button("   Close   ");
		...
		if (imp.getStackSize()>1) {
			addComponent(pnMain, 16, 0, 1, 1, 5, bnPrevS);
			addComponent(pnMain, 16, 1, 1, 1, 2, new Label("   Slice"));
			addComponent(pnMain, 16, 2, 1, 1, 5, bnNextS);
		}

//		addComponent(pnMain, 16, 2, 1, 1, 4, bnClose);
		addComponent(pnMain, 17, 1, 1, 1, 4, bnHelp);
...
//		bnClose.addActionListener(this);
	...
	public synchronized  void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == bnNextS){
			...
			IJ.showMessage("Help","Align RGB planes v1.7 by G.Landini\n"+
