		...
		for (int ss=1; ss<=d; ss++)
			...
			for (int x=0; x<w; x++)
				...
				for (int y=0; y<h; y++)
					...
					redPix = (int)(255*(ipR.getPixelValue(x,y)-redMin)/(redMax-redMin));
					greenPix = (int)(255*(ipG.getPixelValue(x,y)-greenMin)/(greenMax-greenMin));
					bluePix = (int)(255*(ipB.getPixelValue(x,y)-blueMin)/(blueMax-blueMin));
					//scaledPix = 
					greyPix =  (int)(255*(ipGr.getPixelValue(x,y)-greyMin)/(greyMax-greyMin));
					
					if(redPix<(int)0) redPix=0;
					if(greenPix<(int)0) greenPix=0;
					if(bluePix<(int)0) bluePix=0;

					greyPix=(int)(greyPix-(scale*(redPix+bluePix+greenPix)));
