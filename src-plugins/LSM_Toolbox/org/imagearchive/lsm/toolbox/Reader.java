...
import org.imagearchive.lsm.toolbox.info.CZ_LSMInfoExtended;
...
public class Reader {
	...
	public ImagePlus open(String arg, boolean verbose) {
		...
		if (file != null) {
			...
			// IJ.open(file.getParent()+System.getProperty("file.separator")+file.getName());
			OpenDialog.setDefaultDirectory(file.getParent());
		}
		return imp;
	}

	private void showEventList(LSMFileInfo lsm) {
		ArrayList<ImageDirectory> imDirs = lsm.imageDirectories;
		ImageDirectory imDir = imDirs.get(0);
		CZ_LSMInfoExtended cz = (CZ_LSMInfoExtended) imDir.TIF_CZ_LSMINFO;
		EventList events = cz.eventList;
		if (events != null) {
			String header = new String(
					"Time (sec) \tEvent Type \tEvent Description");
			TextWindow tw = new TextWindow("Time Events for " + lsm.fileName,
					header, null, 400, 200);
	...
	public ImagePlus open(String directory, String filename,
		...
		// read using the LSM_Reader
		org.imagearchive.lsm.reader.Reader r = new org.imagearchive.lsm.reader.Reader();
		imp = r.open(directory + System.getProperty("file.separator")
				+ filename, false);
		//read METADATA
		//updateMetadata(imp);
		return imp;
	...
	private CZ_LSMInfoExtended getCZ_LSMINFO(RandomAccessStream stream,
			long position, boolean thumb) {
		CZ_LSMInfoExtended cz = new CZ_LSMInfoExtended();
		try {
			if (position == 0)
				return cz;
